...
function SendSingleTicket(pTicket: ITicketDatabase, pCallBack: (_foo: ITicket[]) => void) {
    fetch(window.location.origin+"/ticket/" + pTicket.id, {
        method: "PUT",
        mode: "cors",
        cache: "no-cache",
...
export async function GetComments(pTicketId:number, pCallBack=(_foo:CommentType[])=>{}){
    const response = await fetch(window.location.origin+"/comment?ticket_id="+pTicketId);
...
export function RemoveCommentTicket(pTicketId:number, pCommentId:number,
                                    pCallBack:Dispatch<SetStateAction<CommentType[]>>){
    fetch(window.location.origin+"/comment/"+pCommentId+"?ticket_id="+pTicketId, {
        method: "DELETE",
        mode: "cors",
        cache: "no-cache",
        credentials: "same-origin",
...
export function LikeCommentTicket(pUserId:number, pTicketId:number, pCommentId:number,
    pCallBack:Dispatch<SetStateAction<CommentType[]>>){
    fetch(window.location.origin+"/comment/like/" + pCommentId+"?ticket_id="+pTicketId, {
        method: "PUT",
        mode: "cors",
        cache: "no-cache",
        credentials: "same-origin",
